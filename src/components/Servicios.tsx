  ...
  return (
    <section id="servicios" className="bg-gray-50 py-16 px-4">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold text-center mb-10 text-gray-800">{messages.services.title}</h2>
        <div className="w-16 h-[2px] bg-yellow-500 rounded-full mb-6 mx-auto" />

        <Swiper modules={[Autoplay, Pagination]} spaceBetween={24} slidesPerView={1} pagination={{ clickable: true }} autoplay={{ delay: 5000 }} breakpoints={{ 768: { slidesPerView: 2 }, 1024: { slidesPerView: 3 } }}>
          {messages.services.items.map((servicio: any, index: number) => (
            <SwiperSlide key={index}>
              <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition duration-300">
                <div className="w-full h-48 relative">
                  <Image src={serviceImages[index]} alt={servicio.title} fill className="object-cover" />
                </div>
